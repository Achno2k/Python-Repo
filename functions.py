...
def emoji_convertor(message):
    words = message.split(' ')      #This split method makes us a new list with all the strings before each space
    print(words)
    emojis = {
    ":)" : "😊",
    ":(" : "😔"
    ...
    output = ""
    for word in words:
        output += emojis.get(word, word) + " "  #emojis.get(__key to search__, __default_value__)
        print(output)
    return output
